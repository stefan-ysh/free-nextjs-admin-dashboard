...
import { releaseReservedStock, INVENTORY_ERRORS } from '@/lib/db/inventory';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error instanceof Error) {
      ...
      if (error.message === INVENTORY_ERRORS.RESERVE_EXCEEDS) {
        return NextResponse.json({ error: '释放数量超过已预留库存' }, { status: 409 });
      }
    }
    return NextResponse.json({ error: '释放预留库存失败' }, { status: 500 });
  }
}
