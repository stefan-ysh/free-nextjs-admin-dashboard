		...
		<Dialog open onOpenChange={(open) => !open && onClose()}>
			<DialogContent className="max-h-[90vh] overflow-hidden p-0 sm:max-w-5xl">
				<ModalShell
					...
					description={<p className="text-xs uppercase tracking-wide text-muted-foreground">采购详情 · {purchase.purchaseNumber}</p>}
					headerActions={
						...
						<div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-xs text-rose-700 dark:border-rose-900/60 dark:bg-rose-950/40 dark:text-rose-200">
							...
							{onReloadDetail ? (
								<Button
									...
									className="text-rose-600 hover:bg-rose-100 dark:border-rose-300 dark:text-rose-100"
								>
									重试
								</Button>
							) : null}
